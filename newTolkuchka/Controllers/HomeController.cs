    ...
    public class HomeController : AbstractHomeController
    {
        ...
        public async Task<IActionResult> Products(string model, string id, bool productsOnly, int[] t, int[] b, string[] v, int minp, int maxp, Sort sort, int page, int pp = 60, string search = null)
        {
            if (string.IsNullOrEmpty(model))
                return await GetNotFoundPageAsync();
            //int? sw = GetScreenWidth(); not used to get products, used only to save a key, if products the method return do not depend on sw why we need it
            ...
            JsonResult result = await ProductsBase(model, id, productsOnly, t, b, v, minp, maxp, sort, page, pp, search);
            return result;
        }
