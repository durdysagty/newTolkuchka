        ...
        if (response.ok) {
            ...
            if (result.noOrders)
                ...
                result.orders.forEach(o => {
                    const tr = $(`<tr><td class='align-middle p-0' style='min-width: 50px'><a href='/product/${o.id}'><img class='img-fluid' style='height: 50px; width: auto' src='${o.image}' /></a></td><td class='align-middle'><a href='/product/${o.id}'>${o.productName}</a> ${o.freeQuantity !== null ? `<span id=${'free' + o.id} ${o.quantity >= o.freeQuantity ? 'class=\"d-block\"' : 'class=\"d-none\"'}>+1  ${stringListCart.freeOne}</span>` : ''} ${o.freeProductQuantity !== null ? `<span id=${'freeQ' + o.id} ${o.quantity >= o.freeProductQuantity ? 'class=\"d-block\"' : 'class=\"d-none\"'}>+ ${o.freeProductName} - ${stringListCart.freeOne}</span>` : ''}  ${o.setId !== null ? `<span>+ ${o.setFreeProductName} - ${stringListCart.freeOne}</span>` : ''}</td><td class='align-middle'  id='${'p' + o.id}'>${o.price}</td><td class='align-middle text-center'>${o.subjected ? '' : `<span role="button" class='pointer px-2' onclick='quantity(${o.id}, false)'>-</span><span id='${'q' + o.id}' > ${o.quantity}</span > <span role="button" class='pointer px-2' onclick='quantity(${o.id}, true)'>+</span>`}</td><td class='align-middle'><span id='${' s' + o.id}'>${o.amount}</span></td><td class='align-middle'><button class='btn btn-primary py-1 px-3' onclick='removeOrder(${o.id})'>X</button></td></tr> `)
                    $('#cartBody').append(tr)
